...
int PetscParallelConfiguration::computeRankFromIndices(int i, int j,
		int k) const {

	if (i < 0 || i >= _parameters.parallel.numProcessors[0] || j < 0
			|| j >= _parameters.parallel.numProcessors[1] || k < 0
			|| k >= _parameters.parallel.numProcessors[2]) {
...
void PetscParallelConfiguration::computeSizes() {
	...
	for (int i = 0; i < dim; i++) {
		for (int j = 0; j < _parameters.parallel.numProcessors[i]; j++) {
			_parameters.parallel.sizes[i][j] = geometrySizes[i]
					/ _parameters.parallel.numProcessors[i];
	...
	for (int i = 0; i < dim; i++) {
		...
		for (int j = 0; j < _parameters.parallel.indices[i]; j++) {
			_parameters.parallel.firstCorner[i] +=
					_parameters.parallel.sizes[i][j];
		}
	}
	if (dim == 2) {
