...
void TurbulentViscosityStencil::apply( FlowField & flowField, int i, int j, int k ) {
    ...
    if ( ( obstacle & OBSTACLE_SELF ) == 0 ) {
        ...
        if ( FieldStencil<FlowField>::_parameters.turbulent.turbulent_scenario != "laminar" ) {
            mixing_length = FieldStencil<FlowField>::_parameters.turbulent.kappa
                    * flowField.getDistanceToNearWall().getScalar( i, j, k );
            if ( FieldStencil<FlowField>::_parameters.turbulent.turbulent_scenario
                    == "turbulent_laminar_plate"
                    || FieldStencil<FlowField>::_parameters.turbulent.turbulent_scenario
                            == "turbulent_turbulent_plate" ) {
        ...
        S11 = ( flowField.getVelocity().getVector( i + 1, j, k )[0]
                - flowField.getVelocity().getVector( i - 1, j, k )[0] )
                / ( 2 * FieldStencil<FlowField>::_parameters.geometry.dx );
        S22 = ( flowField.getVelocity().getVector( i, j + 1, k )[1]
                - flowField.getVelocity().getVector( i, j - 1, k )[1] )
                / ( 2 * FieldStencil<FlowField>::_parameters.geometry.dy );
        S33 = ( flowField.getVelocity().getVector( i, j, k + 1 )[2]
                - flowField.getVelocity().getVector( i, j, k - 1 )[2] )
                / ( 2 * FieldStencil<FlowField>::_parameters.geometry.dz );
        ...
        flowField.getViscosity().getScalar( i, j, k ) = mixing_length * mixing_length
                * sqrt( 2 * Sij2 );
    }
