	...
	public:
		...
		void apply ( FlowField & flowField, int i, int j, int k ) {
			...
			if (_parameters.turbulent.turbulent_scenario == "turbulent_laminar_plate"){
				flowField.getDelta().getScalar (i, j, k) = 4.91 * pow( (_parameters.geometry.dx * i) , 0.5) / pow( (_parameters.flow.Re ), 0.5);
			}
			else if (_parameters.turbulent.turbulent_scenario == "turbulent_turbulent_plate"){
				flowField.getDelta().getScalar (i, j, k) = 0.382 * pow( (_parameters.geometry.dx * i) , 0.8) / pow( (_parameters.flow.Re ), 0.2);
			}
		}
};
